...
class ModelTraining:
    ...
    def save_pickle_files(self, state: TrainingState):
        try:
            ...
            best_metrics = state.model_metrics[state.best_model_name]
            metadata = {
                'timestamp': timestamp,
                'best_model_name': state.best_model_name,
                'best_model_params': json.dumps(state.best_params),
                'best_accuracy': best_metrics['accuracy'],
                'best_precision': best_metrics['precision'],
                'best_recall': best_metrics['recall'],
                'best_f1_score': best_metrics['f1_score'],
                'best_cv_score': best_metrics.get('best_cv_score'),
    ...
    def save_metrics_to_csv(self, state: TrainingState, output_dir: str):
        ...
        for model_name, metrics in state.model_metrics.items():
            detail_path = os.path.join(
                observations_dir, f"{model_name}_detailed_metrics.csv"
            )
            detail_df = pd.DataFrame([{
                ...
                'Best_Parameters': json.dumps(metrics.get('best_params', {})),
            }])
    ...
    def train_models(self, state: TrainingState, cv_folds: int = 5) -> TrainingState:
        ...
        if state.X_train_tfidf is None or state.X_test_tfidf is None:
            raise ValueError("TF-IDF features not found in state. Run DataTransformation first.")
        if state.y_train is None or state.y_test is None:
            raise ValueError("Labels not found in state. Run DataTransformation first.")
        if cv_folds < 2:
        ...
        try:
            ...
            for model_name, model in models.items():
                ...
                if not param_grid:
                    logger.warning(f"No param grid for {model_name}; training with defaults only.")
                ...
                try:
                    ...
                    logger.error(f"{model_name} failed during fit: {exc}")
                    continue
                ...
                try:
                    ...
                    logger.error(f"{model_name} failed during predict: {exc}")
                    continue
            ...
            if not model_metrics:
                raise RuntimeError("All model trainings failed; no best model could be selected.")
